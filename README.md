...
|   |   |-- dto/
|   |   |-- providers/
|   |   |   |-- metadata-multimodal.provider.ts
|   |   |   `-- multimodal.provider.ts
...
-> StorageService
-> MultimodalProvider
-> MultimodalAttachment.extractedText
-> Agent attachmentIds
-> Multimodal Context
-> PromptBuilder
```
...
它只生成文件元数据文本，不伪造 OCR 或 ASR 结果。真实图片识别、OCR、语音识别和视频理解后续通过 `MultimodalProvider` 接口替换，不需要让 Agent 直接处理文件或 buffer。
