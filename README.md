...
当前边界：已实现 Knowledge Space、Document 元数据、对象存储、Document 上传编排、Document 解析、Markdown 分块、Chunk 向量化、Hybrid Retrieval、RRF 融合、Reranker 重排、Context token 裁剪、LLM Chat Completion 和 Conversation/Message 持久化；尚未实现 Redis Memory、Mem0、Agent、LangGraph、Knowledge Graph 或 Tool Calling。
...
- 当前阶段不实现 Redis Memory、Mem0、Agent、LangGraph、Knowledge Graph 或 Tool Calling。
...
-> RetrievalService
-> ContextChunk[]
-> PromptBuilder(History + Context + Question)
-> LlmProvider
