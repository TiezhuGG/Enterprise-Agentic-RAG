    ...
    <Card>
      <CardHeader className="document-preview-panel__header">
        <div>
          ...
          <CardDescription>
            {selectedDocument
              ? 'Preview parsed content produced by the ingestion pipeline.'
              : 'Select a document first.'}
          </CardDescription>
        </div>
        <Badge variant={documentPreview?.parsedContent.available ? 'success' : 'secondary'}>
          {documentPreview?.parsedContent.available ? 'Parsed' : 'Empty'}
        </Badge>
      </CardHeader>
      <CardContent className="document-preview-panel">
        ...
        {!selectedDocument ? (
          <div className="document-preview-panel__empty">No document selected.</div>
        ) : (
          <>
            ...
            {loadingDocumentPreview ? (
              <div className="document-preview-panel__empty">Loading preview...</div>
            ) : null}

            {!loadingDocumentPreview && !documentPreview?.parsedContent.available ? (
              <div className="document-preview-panel__empty">
                <Eye />
                <span>Parsed content appears after ingestion finishes.</span>
              </div>
            ) : null}
            ...
            {documentPreview?.parsedContent.available ? (
              <>
                <div className="document-preview-panel__meta">
                  <span>{documentPreview.parsedContent.contentLength} chars</span>
                  {documentPreview.parsedContent.truncated ? <span>Truncated</span> : null}
                  {documentPreview.file.available ? <span>Original file available</span> : null}
                </div>
