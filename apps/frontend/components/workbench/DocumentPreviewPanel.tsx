    ...
    <Card className="min-w-0">
      <CardHeader className="document-preview-panel__header">
        <div className="min-w-0">
          ...
          <CardDescription>
            {selectedDocument ? '查看已提取的文本内容和检索前的解析结果。' : '请先选择一份文档。'}
          </CardDescription>
        </div>
        <Badge variant={documentPreview?.parsedContent.available ? 'success' : 'secondary'}>
          {documentPreview?.parsedContent.available ? '可预览' : '暂无内容'}
        </Badge>
      </CardHeader>
        ...
        {!selectedDocument ? (
          ...
          <>
            <div className="document-preview-panel__toolbar">
              <div>
                <FileText />
                <span title={selectedDocument.title}>{selectedDocument.title}</span>
              </div>
              <Button
