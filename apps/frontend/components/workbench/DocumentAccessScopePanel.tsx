...
export function DocumentAccessScopePanel() {
  ...
  const currentMember = selectedSpace?.members.find((member) => member.userId === authUser?.id)
  ...
  const [departmentIds, setDepartmentIds] = useState<string[]>(effectiveScope.allowedDepartmentIds ?? []);
  const nextScope = useMemo(() => toScope(mode, departmentIds), [departmentIds, mode]);
  const isDirty = JSON.stringify(nextScope) !== JSON.stringify(toScope(getMode(effectiveScope), effectiveScope.allowedDepartmentIds ?? []));
  ...
  const toggleDepartment = (departmentId: string) => {
    setDepartmentIds((current) => current.includes(departmentId)
    ...
    <Card className="min-w-0">
      <CardHeader className="document-access-scope__header">
        <div className="min-w-0">
          ...
          <CardDescription>{selectedDocument ? '成员身份始终是第一道边界；文档策略只会进一步收紧可访问范围。' : '请先选择一份文档。'}</CardDescription>
        </div>
      ...
      <CardContent>
        ...
        {!selectedDocument ? <div className="document-access-scope__empty">尚未选择文档。</div> : canManage ? (
          <form className="document-access-scope" onSubmit={handleSubmit}>
            ...
            <label className="document-access-scope__field">
              ...
              <small>{mode === 'MEMBERS' ? '所有已加入本知识库的成员均可访问。' : mode === 'DEPARTMENTS' ? '成员身份与部门归属必须同时满足。' : '成员身份仍是前提，负责人或拥有机密资料权限的成员可访问。'}</small>
            </label>
            {mode === 'DEPARTMENTS' ? <fieldset className="document-access-scope__field"><legend>允许访问的部门</legend><div className="grid max-h-52 gap-2 overflow-y-auto border p-3 sm:grid-cols-2">{departments.map((department) => <label className="flex min-w-0 items-center gap-2 text-sm" key={department.id}><input checked={departmentIds.includes(department.id)} className="size-4 shrink-0" onChange={() => toggleDepartment(department.id)} type="checkbox" /><span className="truncate" title={department.name}>{department.name}</span></label>)}</div><small>选择一个或多个部门，不输入技术标识。</small></fieldset> : null}
            <Button disabled={!isDirty || loadingDocuments || (mode === 'DEPARTMENTS' && departmentIds.length === 0)} type="submit"><Save />保存访问范围</Button>
          </form>
        ) : (
          <div className="document-access-scope__readonly"><ScopeSummary mode={getMode(effectiveScope)} departmentCount={effectiveScope.allowedDepartmentIds?.length ?? 0} /><p>当前为只读成员。请联系知识库负责人 {owner?.user.email ?? '申请调整权限'} 修改文档访问范围。</p></div>
        )}
      </CardContent>
    </Card>
  );
...
function ScopeSummary({ departmentCount, mode }: { departmentCount: number; mode: AccessMode }) {
  const label = mode === 'MEMBERS' ? '知识库成员可见' : mode === 'DEPARTMENTS' ? `指定 ${departmentCount} 个部门可见` : '机密资料';
  return <div className="document-access-scope__summary"><div><ShieldCheck /><span>{label}</span></div><div><LockKeyhole /><span>成员身份始终必需</span></div></div>;
}
