...
export function KnowledgeManagementRoutes({
  routeKey,
  spaceId,
}: {
    ...
    () =>
      spaces.filter(
        (space) =>
          ...
          `${space.name} ${space.description ?? ''} ${space.department?.name ?? ''}`
            .toLowerCase()
    ...
    <div className="grid min-w-0 gap-4">
      ...
      <div className="grid min-w-0 gap-2 border border-border bg-card p-4 md:grid-cols-[minmax(0,1fr)_180px_180px]">
        ...
        <Select onValueChange={(value) => setType(value as typeof type)} value={type}>
          ...
          <SelectContent>
            ...
            {Object.entries(knowledgeBaseTypeLabels).map(([value, label]) => (
      ...
      {filteredSpaces.length === 0 ? (
        <ConsoleEmptyState
          ...
          description="创建后即可配置成员、上传资料，并将检索和问答限定在该知识范围内。"
          icon={Database}
          title="暂无知识库"
        />
      ) : (
        <div className="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {filteredSpaces.map((space) => {
            const role =
              space.members.find((member) => member.userId === authUser?.id)?.role ?? 'VIEWER';
            return (
              <button
                className="grid min-h-48 min-w-0 content-between border bg-card p-4 text-left transition-colors hover:bg-muted"
                key={space.id}
                onClick={() => router.push(buildKnowledgeBaseHref(space.id))}
                type="button"
              >
                <div className="min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <Database className="mt-0.5 size-4 shrink-0 text-primary" />
                    <Badge variant="secondary">{knowledgeBaseTypeLabels[space.type]}</Badge>
                  </div>
                  <h2 className="mt-4 truncate text-base font-semibold" title={space.name}>
                    {space.name}
                  </h2>
                  <p className="mt-1 line-clamp-3 min-h-15 text-sm leading-6 text-muted-foreground">
                    {space.description || '未填写知识库说明。'}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 border-t pt-3 text-xs text-muted-foreground">
                  <span>{space.documentCount} 份文档</span>
                  <span>{space.memberCount} 名成员</span>
                  <span className="truncate" title={space.department?.name}>
                    归属：{space.department?.name ?? '未绑定部门'}
                  </span>
                  <span>
                    你的角色：
                    {role === 'OWNER' ? '负责人' : role === 'EDITOR' ? '编辑者' : '查看者'}
                  </span>
                  <span className="col-span-2">
                    更新于 {formatKnowledgeBaseDate(space.updatedAt)}
                  </span>
                </div>
              </button>
            );
...
function KnowledgeBaseDetailPage({ spaceId }: { spaceId?: string }) {
  ...
  const currentMember =
    ...
    (isSynchronized ? spaceMembers.find((member) => member.userId === authUser?.id) : null) ??
    null;
  ...
  return (
    <div className="grid min-w-0 gap-4">
      <ConsolePageHeader
        actions={
          selectedSpace && isSynchronized ? (
            <>
              <Button
                onClick={() =>
                  router.push(buildConsoleHref('documents', { space: selectedSpace.id }))
                }
                variant="outline"
              >
                <FileText />
              ...
              <Button
                onClick={() =>
                  router.push(buildConsoleHref('document-access', { space: selectedSpace.id }))
                }
                variant="outline"
              >
                <ShieldCheck />
          ...
          <section className="grid min-w-0 gap-4 border border-border bg-card p-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div className="min-w-0">
              <div className="flex min-w-0 items-center gap-2">
                <h2 className="truncate text-base font-semibold" title={selectedSpace.name}>
                  {selectedSpace.name}
                </h2>
                <Badge variant="secondary">{knowledgeBaseTypeLabels[selectedSpace.type]}</Badge>
              </div>
              <p className="mt-1 max-w-3xl break-words text-sm text-muted-foreground">
                {selectedSpace.description || '未填写知识库说明。'}
              </p>
            </div>
            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
              <span>归属部门：{selectedSpace.department?.name ?? '未绑定部门'}</span>
          ...
          <Card className="min-w-0 border-destructive/30">
            <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-muted-foreground">
                {canDelete
                  ? '你是知识库负责人，可以删除该知识库。系统管理员身份不会替代知识库成员角色。'
                  : '仅知识库负责人可以删除知识库；系统管理员身份不会自动获得此权限。'}
              </p>
              {canDelete ? (
      ...
      <Dialog onOpenChange={setDeleteOpen} open={deleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              此操作会停止该知识库的日常访问。请输入知识库名称“{selectedSpace?.name}”确认。
            </DialogDescription>
          </DialogHeader>
          <Input
          ...
          <DialogFooter>
            ...
            <Button
              disabled={!selectedSpace || confirmation.trim() !== selectedSpace.name}
  ...
  return (
    <div className="grid min-w-0 gap-4">
      <ConsolePageHeader
        actions={
          <Select
            onValueChange={(value) => setStatus(value as PipelineJobStatus | 'ALL')}
            value={status}
          >
      ...
      <Card className="min-w-0">
        <CardContent className="grid gap-3 p-4">
          {!selectedSpaceId ? (
            <ConsoleEmptyState
              description="请先在顶部选择知识库，再查看其入库任务。"
              icon={FileArchive}
              title="尚未选择知识库"
            />
          ) : null}
          ...
          {!loading && selectedSpaceId && jobs.length === 0 ? (
            <ConsoleEmptyState
              description="上传文档后会自动创建入库任务。"
              icon={FileArchive}
              title="当前知识库暂无入库任务"
            />
          ) : null}
          {jobs.map((job) => {
            ...
            return (
              <article
                className="grid min-w-0 gap-2 border-b border-border pb-3 text-sm last:border-0 last:pb-0 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"
                key={job.id}
              >
                <div className="min-w-0">
                  <p className="truncate font-medium">{job.document.title}</p>
                  <p className="mt-1 truncate text-xs text-muted-foreground">
                    {job.latestEvent?.stage ?? '等待处理'} · 第 {job.attemptCount} 次尝试 ·{' '}
                    {formatDateTime(job.updatedAt)}
                  </p>
                  {graphFailed ? (
                    <p className="mt-1 text-xs text-destructive">
                      图谱抽取失败：{job.graphEvent?.errorMessage ?? '大模型或图谱服务不可用'}
                    </p>
                  ) : job.graphEvent?.status === 'SUCCEEDED' ? (
                    <p className="mt-1 text-xs text-emerald-700">图谱抽取成功</p>
                  ) : null}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    {pipelineStatusLabels[job.status]}
                  </ConsoleStatusBadge>
                  {job.status === 'QUEUED' ? (
                    <Button
