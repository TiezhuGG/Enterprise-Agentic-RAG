  ...
  return (
    <Dialog open={required}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>请先修改临时密码</DialogTitle>
          <DialogDescription>为保护组织资料，请在继续使用控制台前设置新密码。</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid min-h-14 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 md:grid-cols-[minmax(180px,0.6fr)_minmax(260px,1fr)_minmax(220px,320px)_auto] md:px-6 xl:px-8">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                className="hidden min-w-0 justify-between gap-2 px-3 md:flex"
                size="sm"
                variant="outline"
              >
            ...
            <DropdownMenuContent align="end" className="w-80">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onSelect={() => setSpaceCreationOpen(true)}>
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-sidebar">
      <button
        aria-label="返回工作概览"
        className={cn(
          'flex items-center gap-3 border-b border-border px-3 py-3 text-left',
        ...
        {consoleNavigationGroups.map((group) => {
          const items = consoleNavigationItems.filter(
            (item) =>
              ...
              (item.group !== 'governance' ||
                item.key === 'document-access' ||
          ...
          return (
            <div className="mb-4" key={group.key}>
              <p
                className={cn(
                  'mb-1 px-2 text-[11px] font-medium text-muted-foreground',
                ...
                {items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={cn('truncate', compact && 'hidden xl:block')}>
                        {item.label}
                      </span>
                    </button>
                  );
...
function sectionLabel(routeKey: ConsoleRouteKey) {
  return (
    consoleNavigationGroups.find((group) =>
