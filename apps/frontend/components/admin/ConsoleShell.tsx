    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="grid min-h-screen grid-cols-1 md:grid-cols-[64px_minmax(0,1fr)] xl:grid-cols-[232px_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          ...
          <section
            className={cn(
              'min-w-0 px-4 py-5 md:px-6 md:py-6 xl:px-8',
              isWorkbench ? 'w-full' : 'mx-auto w-full max-w-[1360px]',
            )}
          >
            {children}
          </section>
      ...
      <div className="grid min-h-14 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 md:grid-cols-[minmax(180px,0.6fr)_minmax(260px,1fr)_minmax(220px,320px)_auto] md:px-6 xl:px-8">
        ...
        <form className="hidden min-w-0 md:block" onSubmit={handleSearch}>
          <div className="relative max-w-xl">
            ...
            <Input
              className="h-8 border-transparent bg-muted pl-9 pr-14 shadow-none focus-visible:border-ring"
              onChange={(event) => setGlobalQuery(event.target.value)}
              placeholder="搜索文档、问题或执行记录"
              value={globalQuery}
            />
            <span className="pointer-events-none absolute right-2 top-1/2 hidden -translate-y-1/2 border border-border bg-card px-1.5 py-0.5 text-[10px] text-muted-foreground xl:inline">
              Enter
            </span>
          </div>
        </form>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-80">
                ...
                <DropdownMenuItem className="items-start gap-2" key={space.id} onClick={() => void handleSpaceChange(space.id)}>
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-xs text-muted-foreground">
                      {space.visibility === 'PRIVATE' ? '私有空间' : space.visibility === 'PUBLIC' ? '公开空间' : '内部空间'}
                    </span>
                  </span>
            ...
            <form className="grid gap-2 p-2" onSubmit={handleCreateSpace}>
              <Input onChange={(event) => setNewSpaceName(event.target.value)} placeholder="新建知识空间" value={newSpaceName} />
        ...
        <div className="flex items-center gap-1">
          <Button onClick={() => void refresh()} size="icon" title="刷新状态" variant="ghost">
            <RefreshCw />
            <span className="sr-only">刷新状态</span>
          </Button>
          <Button size="icon" title="帮助中心" variant="ghost">
            <CircleHelp />
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button className="gap-2 px-2" variant="ghost">
                ...
                <span className="hidden max-w-32 truncate text-sm lg:block">{authUser?.email ?? '管理员'}</span>
                <ChevronDown className="size-4 text-muted-foreground" />
              </Button>
            </DropdownMenuTrigger>
...
function ConsoleSideNav({ compact = false, routeKey }: { compact?: boolean; routeKey: ConsoleRouteKey }) {
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-sidebar">
      <button
        aria-label="返回工作概览"
        className={cn('flex items-center gap-3 border-b border-border px-3 py-3 text-left', compact && 'justify-center xl:justify-start xl:px-4')}
      ...
      >
        ...
        <span className={cn('min-w-0', compact && 'hidden xl:block')}>
          ...
          <span className="block truncate text-[11px] text-muted-foreground">Knowledge Operations</span>
        </span>
      </button>
        ...
        {consoleNavigationGroups.map((group) => {
          ...
          return (
            <div className="mb-4" key={group.key}>
              <p className={cn('mb-1 px-2 text-[11px] font-medium text-muted-foreground', compact && 'hidden xl:block')}>
                {group.label}
              </p>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground',
                        compact && 'justify-center px-2 xl:justify-start xl:px-2.5',
                    ...
                    >
                      ...
                      <span className={cn('truncate', compact && 'hidden xl:block')}>{item.label}</span>
                    </button>
                  );
                })}
              </div>
        ...
        <div className="border border-border bg-muted/40 p-3 text-xs">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <div className="border border-border bg-card px-2 py-1.5">
              <div className="text-sm font-semibold text-foreground">{spaces.length}</div>
              <div className="text-[11px] text-muted-foreground">空间</div>
            </div>
            <div className="border border-border bg-card px-2 py-1.5">
              <div className="text-sm font-semibold text-foreground">{documents.length}</div>
...
function sectionLabel(routeKey: ConsoleRouteKey) {
  return consoleNavigationGroups.find((group) =>
    consoleNavigationItems.some((item) => item.key === routeKey && item.group === group.key),
  )?.label ?? '控制台';
}
